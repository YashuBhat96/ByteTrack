...
def grid_search(train_dataset, val_dataset, device, hyperparameter_grid):
    ...
    for optimizer_type in hyperparameter_grid['optimizer']:
        for patience in hyperparameter_grid['patience']:
            logger.info(f"Testing combination: Optimizer={optimizer_type}, Epochs=10, Patience={patience}")
            ...
            for epoch in range(10):
                ...
                if early_stopping(val_losses, patience):
                    logger.info(f"Early stopping triggered for combination: Optimizer={optimizer_type}, Epochs=10, Patience={patience}")
                    break

            if min(val_losses) < best_val_loss:
                best_val_loss = min(val_losses)
                best_hyperparameters = (optimizer_type, 10, patience)
...
if __name__ == '__main__':
    ...
    logger.info(f"Best Hyperparameters: Optimizer: {best_hyperparameters[0]}, Epochs: {best_hyperparameters[1]}, Patience: {best_hyperparameters[2]}")
