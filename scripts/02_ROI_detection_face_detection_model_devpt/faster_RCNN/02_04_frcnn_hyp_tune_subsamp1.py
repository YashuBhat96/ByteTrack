...
train_sample_dataset = FaceDataset('/storage/group/klk37/default/homebytes/video/fbs/split_resized/train_sample', transform=transform)
logger.info("Train sample dataset size: %d", len(train_sample_dataset))

train_trans_sample_dataset = FaceDataset('/storage/group/klk37/default/homebytes/video/fbs/split_resized/train_trans_sample', transform=transform)
logger.info("Train transformed sample dataset size: %d", len(train_trans_sample_dataset))

val_sample_dataset = FaceDataset('/storage/group/klk37/default/homebytes/video/fbs/split_resized/val_sample', transform=transform)
...
def grid_search(train_dataset, val_dataset, device, hyperparameter_grid):
    best_val_loss = float('inf')
    best_hyperparameters = None

    for lr in hyperparameter_grid['learning_rate']:
        for bs in hyperparameter_grid['batch_size']:
            for acc_steps in hyperparameter_grid['accumulation_steps']:
                for epochs in hyperparameter_grid['epochs']:
                    logger.info(f"Testing combination: LR={lr}, BS={bs}, AccSteps={acc_steps}, Epochs={epochs}")
                    ...
                    patience = 2 if epochs == 5 else 3
                    val_losses = []

                    for epoch in range(epochs):
                        train_one_epoch(model, optimizer, train_loader, device, epoch, print_freq=10, accumulation_steps=acc_steps)
                        lr_scheduler.step()
                        val_loss = evaluate_loss(model, val_loader, device)
                        val_losses.append(val_loss)
                        
                        if early_stopping(val_losses, patience):
                            logger.info(f"Early stopping triggered for combination: LR={lr}, BS={bs}, AccSteps={acc_steps}, Epochs={epochs}")
                            break

                    if min(val_losses) < best_val_loss:
                        best_val_loss = min(val_losses)
                        best_hyperparameters = (lr, bs, acc_steps, epochs)
...
if __name__ == '__main__':
    ...
    best_hyperparameters, best_val_loss = grid_search(combined_train_sample_dataset, val_sample_dataset, device, hyperparameter_grid)
    
    logger.info(f"Best Hyperparameters: Learning Rate: {best_hyperparameters[0]}, Batch Size: {best_hyperparameters[1]}, Accumulation Steps: {best_hyperparameters[2]}, Epochs: {best_hyperparameters[3]}")
