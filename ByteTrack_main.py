...
# Define the path to yolov7_usage_ByteTrack relative to the script directory
yolo_code_dir = os.path.join(script_dir, "yolov7_usage_ByteTrack")

# Add yolo_code_dir to system path
...
def initialize_models(yolo_model_path, faster_rcnn_model_path, lstm_model_path):
    ...
    yolo_model.half()  # Use half-precision if CUDA is available for faster inference
...
def confirm_bite_with_lightweight_motion(cap):
    """
...
# Function to save results with output path from configuration
def save_results(video_name, bite_count, frame_number, confidence, overall_bite_rate, current_bite_rate, start_time, end_time, latency_seconds):
    ...
    with open(output_file, 'a', newline='') as csvfile:
        ...
        if not file_exists:
            ...
            writer.writerow([
                "Bite Count", "Frame Number", "Confidence", "Start Time (s)", "End Time (s)", 
                "Overall Bite Rate (bites/min)", "Current Bite Rate (bites/min)", "Latency (seconds)"
            ])
        # Write the data row
        writer.writerow([
            bite_count, frame_number, f"{confidence:.4f}", f"{start_time:.2f}", f"{end_time:.2f}",
            f"{overall_bite_rate:.4f}", f"{current_bite_rate:.2f}", f"{latency_seconds:.4f}"
        ])

def log_bite(video_name, bite_count, frame_number, confidence, start_time, end_time, overall_bite_rate, current_bite_rate, output_dir, last_logged_frame, frame_interval=30):
    """
    ...
    if frame_number - last_logged_frame >= frame_interval:
        save_results(video_name, bite_count, frame_number, confidence, overall_bite_rate, current_bite_rate, start_time, end_time, output_dir)
                        ...
                        if len(probabilities) >= window_step:
                            ...
                            if last_smoothed_prob > bite_threshold:
                                ...
                                if valid_chewing:
                                    ...
                                    # Log result
                                    with open(output_file, 'a') as log_file:
                                        log_file.write(f"{bite_count}, {frame_count}, {last_smoothed_prob:.4f}, {frame_capture_time:.2f}, {detection_time:.2f}, {overall_bite_rate:.4f}, {current_bite_rate:.2f}, {latency_seconds:.4f}\n")
...
def process_videos(directory_path, yolo_model, faster_rcnn_model, lstm_model, efficientnet, config):
    """
    Process all videos in the given directory and apply bite detection using configuration parameters.
    """
    output_dir = config['paths']['output_dir']
    processing_params = config['processing']

    for filename in os.listdir(directory_path):
        if os.path.isfile(os.path.join(directory_path, filename)) and filename.lower().endswith(('.mp4', '.avi', '.mov')):
            video_path = os.path.join(directory_path, filename)
            print(f"Processing video: {filename}")
            
            # Call detect_and_track with parameters from the config
...
if __name__ == "__main__":
    ...
    config = load_config(config_path)  # Ensure this line is the first use of 'config'
